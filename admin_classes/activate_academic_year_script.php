...
if(isset($_POST['academic_id'])){
  ...
  $user_action = 'Activated academic year <b>'.$academic_year_from.' - '.$academic_year_to.' '.'('.$academic_term.')</b>';
  ...
  $zero          = 0;
    ...
    mysqli_query($con, "UPDATE academic_year SET academic_status = 1 WHERE academic_id = '$academic_id'");
    mysqli_query($con, "UPDATE academic_year SET academic_status = 0 WHERE academic_id != '$academic_id' AND academic_status != 2");

    header("location:../admin_academic.php?activated&academic_id=$academic_id");
}
