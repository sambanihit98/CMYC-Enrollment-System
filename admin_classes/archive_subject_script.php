...
if(isset($_POST['subject_id'])){
    ...
    $query = mysqli_query($con, "SELECT * FROM manage_subject JOIN manage_program ON manage_subject.program_id = manage_program.program_id
        ...
        $user_action      = 'Archived <b>'.$subject.'</b> subject on <b>'.$program_code.'</b> course (<b>'.$year_level.' - '.$subject_semester.'</b>)';
        ...
        mysqli_query($con, "UPDATE manage_subject SET subject_status = 2 WHERE subject_id = '$subject_id'");

        header("location:../registrar_manage_subject.php?department&department_id=$department_id&curriculum_id=$curriculum_id&subject=$subject&archived");
