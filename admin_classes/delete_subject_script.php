...
$query = mysqli_query($con, "SELECT * FROM manage_subject JOIN manage_program ON manage_subject.program_id = manage_program.program_id
        ...
        $user_action      = 'Deleted <b>'.$subject_description.' '.'('.$subject_code.')</b> subject on <b>'.$program_code.'</b> course (<b>'.$year_level.' - '.$subject_semester.'</b>)';
...
mysqli_query($con, "DELETE FROM manage_subject WHERE subject_id = '$subject_id'");

header("location:../registrar_manage_subject.php?department&department_id=$department_id&curriculum_id=$curriculum_id&subject=$subject&deleted");
